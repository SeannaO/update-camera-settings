...
void getPrimaryPoolId(gchar ** pool_id){
	...
	*pool_id = NULL;
	...
	for(elem = pools; elem; elem = elem->next) {
		...
		syslog(LOG_DEBUG, "Id:[%s] Name:[%s] size:[%ld] rawsize:[%ld] allocated:[%ld] protection:[%d] Status:[%d] Flags:[%d] PercentComplete[%d]", item->id, item->name, (long)item->size, (long)item->rawsize, (long)item->allocated, item->protection, item->status, item->flags, item->percentcomplete);
...
char * systemStoragePoolCreate(){
	...
	for(elem = drives_info->raidtypes; elem; elem = elem->next) {
		...
		syslog(LOG_DEBUG, "Position:[%d] Size:[%ld] ishdd:[%d] description:[%s] firmware:[%s] pool:[%s] Status:[%d]", drive_item->position, (long)drive_item->size, (int)drive_item->ishdd, drive_item->description, drive_item->firmware, drive_item->pool, drive_item->status);
		if (drive_item->ishdd){
...
gboolean shareExists(){
	...
	syslog(LOG_DEBUG, "ShareExists %d", (int) ret);
	return ret;
}
